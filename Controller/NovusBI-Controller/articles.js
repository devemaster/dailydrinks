          ...
          if (err1) {
            ...
            if (res1.rows == "") {
              ...
              const written_on_date =
                ...
                "/" +
                today.getFullYear();
              const sql = `INSERT INTO bi_articles(title,written_on,auther,description,editor,image,embed,quotations,subcat_id,is_status,created_by,created_on) VALUES ('${
                info.title
              }','${written_on_date}','${info.auther}','${info.description}','${
                info.editor
              }','${info.image}','${info.embed}','${info.quotations}','${
                info.subcat_id
              }','${1}','${user.id}','${today}')RETURNING article_id`;
        ...
        client.query(sql, (error, result) => {
          ...
          if (error) {
            ...
            if (result) {
              ...
              redisClient.hdel("bi_articles", article_title, function (
                err,
                redisdata
              ) {
                if (err) {
