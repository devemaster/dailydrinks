      ...
      if (user.role_id == 1 || user.role_id == 2 || user.role_id == 4) {
        ...
        client.query(findcommet, (commeterr, commentress) => {
          if (commeterr) {
            ...
            if (commentress.rows != "") {
              const sql = `INSERT INTO bi_comment(comment,contant_id,user_id,is_status,created_on) VALUES ('${
                info.comment
              }','${info.contant_id}','${
                user.id
              }','${1}','${today}')RETURNING comment_id`;
              ...
              client.query(sql, (error, result) => {
                if (error) {
                  ...
                  if (result) {
                    ...
                    redisClient.hdel("bi_comment", commentId, function (
                      err,
                      redisdata
                    ) {
                      if (err) {
