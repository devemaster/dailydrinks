        ...
        if (contantdat1[0].name == "" || info.category == "") {
          ...
          client.query(Chkcontant, (err1, res1) => {
            if (err1) {
              ...
              if (res1.rows == "") {
                const sql = `INSERT INTO bi_contant(title,contant,type,categories,date,author,higlight,resume,comment,updated_at,status,created_by,pdf,categories_name,thumbnail,created_at,region) VALUES ('${
                  info.title
                }','${contantdat}','${info.type}','${cat_value}','${
                  info.date
                }','${info.author}','${info.heighlight}','${info.resume}','${
                  info.comment
                }','${myDate}','${"draft"}','${user.id}','${info.pdf}','${
                  info.categories_name
                }','${info.thumbnail}','${myDate}','${
                  info.region
                }')RETURNING contant_id`;
            ...
            if (err1) {
              ...
              if (res1.rows == "") {
                ...
                const sql = `INSERT INTO bi_contant(title,contant,type,categories,date,author,higlight,resume,comment,updated_at,status,created_by,pdf,categories_name,thumbnail,created_at,region) VALUES ('${
                  ...
                  info.comment
                }','${myDate}','${"pending"}','${user.id}','${info.pdf}','${
                  info.categories_name
                }','${info.thumbnail}','${myDate}','${
                  info.region
                }')RETURNING contant_id`;
            ...
            if (res.rows != "") {
              ...
              client.query(sql, (error, result) => {
                if (error) {
                  ...
                  const updatecontantdata = `INSERT INTO bi_contant(contant_id,title,contant,type,categories,date,author,higlight,resume,comment,updated_at,status,created_by,pdf,categories_name,thumbnail,created_at,region) VALUES ('${
                    info.contant_id
                  }','${info.title}','${contantdat}','${
                    info.type
                  }','${cat_value}','${info.date}','${info.author}','${
                    info.heighlight
                  }','${info.resume}','${
                    info.comment
                  }','${myDate}','${"pending"}','${user.id}','${info.pdf}','${
                ...
                if (deldataress.rows[0].status == "trace") {
                  ...
                  client.query(sql, (error, result) => {
                    if (error) {
                      ...
                      redisClient.hdel("bi_contant", contant_title, function (
                        err,
                        redisdata
                      ) {
                        if (err) {
        ...
        client.query(searchcat, (searchcaterr, searchcatress) => {
          if (searchcaterr) {
            ...
            if (searchcatress.rows == "") {
              ...
              for (let checktrace of searchcatress.rows) {
                if (
                  checktrace.status != "trace" &&
                  checktrace.categories_name != "Podcast"
                ) {
