        ...
        if (email && password) {
          ...
          client.query(checkAdmin, (adminerr, adminresult) => {
            if (adminresult.rows != "") {
              bcrypt.compare(
                password,
                adminresult.rows[0].password.trim(),
                (err, isMatch) => {
                  if (isMatch == true) {
                    var token = jwt.sign(
                      {
                    ...
                    redisClient.hget("user", email, function (err, reply) {
                      ...
                      if (reply == null) {
                        ...
                        bcrypt.compare(
                          ...
                          (err, isMatch) => {
                            if (isMatch == true) {
                              const appuser = `select * from application_management where application_id = '${
                                ...
                                if (apperr) {
                                  ...
                                  if (appress.rows == "") {
                                    ...
                                    appdata = {
                                      ...
                                      created_date:
                                        appress.rows[0].created_date,
                                    };

                                    redisClient.hget(
                                      "application",
                                      appress.rows[0].application_name,
                                      function (err1, redress1) {
                                        if (err1) {
                                          resolve(message.SOMETHINGWRONG);
                                        } else {
                                          const Admindata = {
