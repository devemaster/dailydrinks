...
const redisClient = require("../../cache").createClient();
...
const { URL, Password, Username } = require("../../lib/config");
...
var sendEmailToSignup = (email, company, fullname) => {
  nodemailer.createTestAccount((err, account) => {
    ...
    let mailOptions = {
      ...
      html:
        '<div style="height: 35px; width: 100%; background-color: purple; text-align: center; color: white; padding-top: 15px; font-weight: bold;">NovusOne Team</div><br><br>Dear <b style="color:red">' +
        fullname +
        '</b> Administrator,<br><br>&nbsp;&nbsp;&nbsp;<p style="color:blue">You are created as user for ' +
        company +
        ' company.<br><br>Thank You <br>NovusOne team<br><br><br><div style="height: 45px; width: 100%; background-color: purple; text-align: center; color: white"></div>',
    };
...
// ====================== userlogin ========================= //
var addUserToRedis = (body, device_type, device_token) => {
  return new Promise((resolve, reject) => {
    try {
      let redata = {
      ...
      redisClient.hmset(
        ...
        function (err, data) {
          if (err) {
            ...
            if (data == "OK") {
              resolve(message.USERADDTOREDIS);
            } else {
              resolve(message.SOMETHINGWRONG);
            }
          }
              ...
              if (result.rows == "") {
                ...
                if (result.rows[0].status == 0) {
                      ...
                      bcrypt.compare(
                        ...
                        (err, isMatch) => {
                          if (isMatch == true) {
                            ...
                            redisClient.hget("user", email, function (
                              err,
                              reply
                            ) {
                              const data = JSON.parse(reply);
                  ...
                  if (result.rows[0].status == 2) {
                    resolve({
                      success: false,
                      message: "User requested disapprove",
                    });
                  } else {
                    resolve({
                      success: false,
                      message: "Your requested not approve",
                    });
                  }
                }
              }
      ...
      if (email != "" && password != "") {
        if (email && password) {
          ...
          client.query(superlogin, (supererr, superresult) => {
            if (superresult.rows != "") {
              if (
                superresult.rows[0].status == 0 &&
                superresult.rows[0].role_id == 1
              ) {
                            ...
                            (err, isMatch) => {
                              if (isMatch == true) {
                                const superdata = {
                                  ...
                                  created_date:
                                    superresult.rows[0].created_date,
                                };
                                const tokenData = {
                                  success: true,
...
var sendEmailToCustomer = (email, password, first_name) => {
  nodemailer.createTestAccount((err, account) => {
    ...
    let mailOptions = {
      ...
      html:
        '<div style="height: 35px; width: 100%; background-color: purple; text-align: center; color: white; padding-top: 15px; font-weight: bold;">NovusOne</div><br><br>Hello <b style="color:red">' +
        first_name +
        '</b>,<br><br>&nbsp;&nbsp;&nbsp;You recently requested to reset your password for your <b style="color:red;">' +
        email +
        "</b> account <br><br>Password : " +
        password +
        '<br><br><div style="height: 45px; width: 100%; background-color: purple; text-align: center; color: white"></div>',
    };
        ...
        if (searcherr) {
          ...
          if (searchress.rows != "") {
            for (let key of searchress.rows) {
              if (
                key.application_name == "Novus BI" ||
                key.application_name == "NovusBI"
              ) {
                const URLPATH = `${URL}novusapp/appuser_login`
                ...
                const URL = process.env.ANIMALHEALTH_API;
                var options = {
                  url: URL,
                  method: "POST",
                  form: {
