...
const { Username, Password,URL} = require('../../lib/config')
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      if (role_id == 1) {
        ...
        const listuser = `select * from signup where role_id = '${4}' and created_by = '${
          user.id
                      ...
                      if (err1) {
                        ...
                        let redata = {
                          ...
                          access_application_id:
                            adminresult.rows[0].access_application_id,
                      ...
                      client.query(updatedata, (updatedataerr, result1) => {
                        if (updatedataerr) {
                          ...
                          let redata = {
                            ...
                            access_application_id:
                              result.rows[0].access_application_id,
                      ...
                      function (err, data) {
                        if (err) {
                          ...
                          if (data == "OK") {
                            ...
                            for (let appData of app_user) {
                              const sqlApp = `insert into app_user(application_id,user_name,user_id,status,role_id) values('${
                                appData.application_id
                              }','${
                                appData.user_name
                              }','${user_id}','${0}','${4}')RETURNING app_id`;
...
var sendResetPasswordEmail = (email, reset_token, first_name) => {
  nodemailer.createTestAccount((err, account) => {
    ...
    let mailOptions = {
      ...
      html:
        `<div style="height: 35px; width: 100%; background-color: purple; text-align: center; color: white; padding-top: 15px; font-weight: bold;">NovusOne</div><br><br>Hello <b style="color:red"> ${first_name}</b>,<br><br>&nbsp;&nbsp;&nbsp;You recently requested to reset your password for your <b style="color:red;">${email}</b> account <br><br>Click on link : <a href="${URL}api/resetpassword/token:${reset_token} +'"><button > Reset Password </button></a><br><br>Thank You <br>NovusOne team<br><br><br><div style="height: 45px; width: 100%; background-color: purple; text-align: center; color: white"></div>`,
    };
              ...
              if (chkress.rows == "") {
                ...
                client.query(del, (delerr, delress) => {
                  if (delerr) {
                    ...
                    redisClient.hdel("signup", rUdata, function (
                      err1,
                      redisdata1
                    ) {
                      if (err1) {
